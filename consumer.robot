...
*** Variables ***
${API_ENDPOINT_POST}=    https://robocorp.com/inhuman-insurance-inc/sales-system-api

*** Tasks ***
Consume traffic data work items
    For Each Input Work Item    keyword_or_func=Process traffic data
...
Validate traffic data
    ...
    ${valid}=    Evaluate    expression= len("${country}") == 3
    [Return]    ${valid}
...
Handle traffic API OK response
    Release Input Work Item    state=DONE

Handle traffic API error response
    [Arguments]    ${return}    ${traffic_data}
    Log
    ...    message= Traffic data posting failed: ${traffic_data} ${return}
    ...    level=ERROR
